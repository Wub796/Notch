...
///
/// Ported verbatim from Glance (`Settings/ProgressiveHeaderBlur.swift`,
/// MIT © Jonathan Zhou).
///
/// SwiftUI's native `scrollEdgeEffectStyle(.soft, for: .top)` (macOS 26) is
/// the system version of exactly this, but it needs macOS 26 — this app's
/// deployment target is 14.0, and the header here is a custom overlay rather
/// than a real toolbar/safe-area inset for that system effect to attach to.
/// So this fakes it manually: each `Material` (weakest to strongest) is masked
/// to fade out over a shorter distance than the last, so only a thin band right
/// at the top ever stacks every layer — the same layered-mask trick most manual
/// "progressive blur" implementations use, since `.blur(radius:)` itself has no
/// per-pixel-varying radius.
