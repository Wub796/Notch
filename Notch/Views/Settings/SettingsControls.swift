...
/// custom toggle, a slider row, an action-button row, the option tiles, the
/// background blur, and the NSWindow chrome configurator.
///
/// Ported from Glance (`Settings/SettingsControls.swift`, MIT © Jonathan Zhou).
/// Adapted in four ways: `GlanceTheme.accent` becomes `SettingsMetrics.accent`,
/// `GlanceToggle` is spelled `SettingsToggle`, the two pickers that Glance
/// points at its own overlay call this app's `FaceIDOverlayController` instead,
/// and Glance's `SettingsRow`/`SettingsActionRow` are left out — this app's
/// own row types in `SettingsComponents.swift` are reskinned onto these same
/// tokens and are already what every pane passes.
...
/// at a fixed width. Shared by every dropdown in Settings, so they all match.
...
/// A title/subtitle pair with a trailing action button, for dropping directly
/// inside a `SettingsGroup` alongside other rows.
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.vertical, 6)
                .background(isDestructive ? Color.red.opacity(0.85) : SettingsMetrics.accent)
                .clipShape(Capsule())
        }
        .buttonStyle(.plain)
        .disabled(!isEnabled)
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .background(SettingsMetrics.accent)
                .clipShape(Capsule())
        }
        .buttonStyle(.plain)
        .disabled(!isEnabled)
...
/// unlocking" or "nothing set up yet" state. Shared by Password and Face
...
/// Section caption used above a group of rows on a settings page.
            ...
            VStack(alignment: .leading, spacing: 2) {
                ...
                if let subtitle {
                    Text(subtitle)
                        ...
                        .foregroundStyle(SettingsMetrics.textTertiary)
                        .fixedSize(horizontal: false, vertical: true)
                        // Same cap as `SettingsRowContent`'s subtitle, so both
                        // wrap at the same width instead of one stretching
                        // further than the other depending on how many tiles
                        // sit beside it.
            ...
            VStack(spacing: 8) {
                preview()
                    ...
                    .background {
                        ...
                        if isSelected {
                            SettingsMetrics.accent.opacity(SettingsMetrics.optionPreviewSelectedTintOpacity)
                        }
                    }
...
/// "Show animation" toggle above it (see the Face ID page's General section).
...
/// Liveness strength picker — connects, inside one `SettingsGroup`, to the
/// "Liveness checks" toggle above it (see the Face ID page's Recognition
/// section). Same title-left/tiles-right shape as `UnlockTriggerPicker`.
struct LivenessModePicker: View {
    ...
    /// A low sun for Light (it only screens *out* spoofs) versus a high one
    /// for Heavy (it also demands positive proof of life).
...
/// Multi-select picker for what arms Face ID — connects, inside one
/// `SettingsGroup`, to the "Enable Face ID" toggle above it. Tiles toggle
/// rather than replace, but the last remaining selection is sticky: with
/// nothing selected the notch would never appear.
...
final class WindowConfiguringView: NSView {
    ...
    private static func configure(_ window: NSWindow) {
        ...
        let toolbar = NSToolbar(identifier: "NotchSettingsToolbar")
