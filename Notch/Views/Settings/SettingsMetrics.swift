...
/// Design tokens for the Settings window — one file to tune sizing/color in
/// rather than scattered literals.
///
/// Ported from Glance (`Settings/SettingsMetrics.swift`, MIT © Jonathan Zhou)
/// so the app's settings window and Glance's are the same object, pixel for
/// pixel: same window size, same row cards, same tab bar, same text ramp. The
...
enum SettingsMetrics {
    ...
    /// lights in it on the leading side, the session lock button sits on the
    /// trailing side.
